...
2.  **Indicator Calculation**: A series of wrapper functions calculate
    ...
    - **Occupancy & Dynamics**: Species occurrences (`spec_occ`),
      ...
      (`relative_occupancy`), and species turnover (`occ_turnover`).
3.  **Uncertainty & Visualization**: Calling the generic
    ...
    or time-series plots. To calculate uncertainty, the user passes a
    temporal indicator object to
    [`add_ci()`](https://b-cubed-eu.github.io/b3gbi/reference/add_ci.md).
    Decoupling the intensive bootstrapping process from initial
    indicator calculations allows for quick data exploration before
    executing heavy simulations. The package includes a minimalist,
    internal spatiotemporal occurrence cube dataset, allowing users to
    execute and verify the entire pipeline out of the box.
