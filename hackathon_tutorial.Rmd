---
title: "Hackathon Project 11: Tutorial"
author: "Shawn Dove"
date: "2024-03-27"
...
The goal of project 11 is to use the R package Shiny to build a front end for an existing (under development) package called b3gbi, which contains a number of functions for calculating and plotting biodiversity indicators from GBIF occurrence cubes.
The package b3gbi is not ready for official release yet, but it is publicly available at https://www.github.com/b-cubed-eu/b3gbi. 

The idea is that the Shiny app will act as an intermediate between the user and the package functions to reach a level of user-friendliness that does not require any coding knowledge.
We will hopefully not need to modify the existing code base to do this, but we will need to interface with some of the functions.
Below I will explain how to install the package on your own computer, and give a quick tutorial on the core functions that we will need to access, with examples. Given that the package is unfinished and has undergone very limited testing, it is possible that you will encounter errors.
...
It's very simple to use. You only need to provide it with the name and location of a cube in .csv format. Note that there are different versions of GBIF cubes floating around, and the format has changed. Since most of the code for this package was created before the present generation came available, it is compatible with both the old and new cubes.
The old ones come with an additional .csv file containing taxonomic information for the cube. I have provided an old cube in the package for the example, but I hope to have a new one available by the time we start.
...
# Example:
library(b3gbi)
europe_cube <- process_cube("data/europe_species_cube.csv", "data/europe_species_info.csv") 
europe_cube
...
There are some additional functions in the package that we may make use of, such as list_species(), which lists the species contained in a cube or indicator object, but they are mostly quite simple and unimportant.

## Shiny

Now that we've had a little tour of the package, let's look at the structure of a basic Shiny app.
...
server <- function(input, output) {
  ...
  output$distPlot <- renderPlot({
    ...
    x    <- example_indicator_map1$data$diversity_val
    x    <- na.omit(x)
    bins <- seq(min(x), max(x), length.out = input$bins + 1)

    hist(x, breaks = bins, col = "#75AADB", border = "black",
         xlab = "Evenness",
         main = "Example Indicator")
...
We can't render the output here because Shiny content is interactive. But this script creates a window with a histogram in the main window on the right, and a slider to control the number of bins in the side panel on the left. The histogram shows the distribution of evenness values across cells in an indicator_map example object included in the b3gbi package.

A Shiny app always requires a ui function for the interactive output that the user sees, and a server function for the back end calculations that the user does not see. Therefore, we will use the ui function to get input from the user and send it to the server function, and to display output from our b3gbi package functions. We will use the server function to define responses to user input and provide any calculations or formatting of output before sending it to ui for display.

Feel free to play around with this script and change how it works.

If you've never used Shiny before, it is probably best to start here:
https://shiny.posit.co/r/getstarted/shiny-basics/lesson1/index.html
or here:
https://ourcodingclub.github.io/tutorials/shiny/

However, please take note: myself and Yanina Sica, who are leading project 11, are NOT Shiny experts. We will be learning as we go. 
So, if you are an expert on Shiny we welcome your experience. If you have no experience with Shiny, you are still welcome to join the project and learn with us, but keep your expectations low.
