...
- rows:
  ...
  (Optional) Choose which row to select if there are multiple matches
  when retrieving taxonomic information from GBIF. (Default is 1. Use NA
...
matrix based on pairwise taxonomic relationships is calculated for each
cell using the taxize package (Chamberlain & Szöcs, 2013; Chamberlain et
al., 2020), then taxonomic distinctness is calculated as the Taxonomic
Distinctness Index (TDI; Clarke & Warwick, 1999): \$\$
\frac{\sum\sum\_{i\<j} \frac{\|R_i - R_j\|}{L}}{\frac{S(S-1)}{2}} \$\$
where S is the number of species, Ri and Rj are the taxonomic ranks of
species i and j (from the GBIF Taxonomic Backbone), and L is the maximum
number of taxonomic ranks. The TDI ranges from 0 to 1, with higher
values indicating greater taxonomic distinctness.
...
Chamberlain, S., Szoecs, E., Foster, Z., Boettiger, C., Ram, K.,
Bartomeus, I., Baumgartner, J., O'Donnell, J., Oksanen, J., Tzovaras, B.
G., Marchand, P., Tran, V., Salmon, M., Li, G., & Grenié, M. (2020).
