...
test_that("add_ci and prepare_indicator_bootstrap errors are covered", {
  ...
  expect_error(
    b3gbi::prepare_indicator_bootstrap(bad_indicator, num_bootstrap = 10, ci_type = "norm"),
    "Unknown indicator"
  )
  ...
  expect_error(
    b3gbi::prepare_indicator_bootstrap(spec_indicator, num_bootstrap = 10, ci_type = "norm"),
...
test_that("calc_ci methods stopifnot_error for wrong data class", {
  ...
  expect_error(b3gbi:::calc_ci.total_occ(data.frame(), indicator_df), "Wrong data class")
  expect_error(b3gbi:::calc_ci.occ_density(data.frame(), indicator_df), "Wrong data class")
  expect_error(b3gbi:::calc_ci.spec_richness_density(data.frame(), indicator_df), "Wrong data class")
  
  x_newness <- data.frame(year=2000)
  expect_error(b3gbi:::calc_ci.newness(x_newness, indicator_df), "Wrong data class")
  
  expect_error(b3gbi:::calc_ci.williams_evenness(data.frame()), "Wrong data class")
  expect_error(b3gbi:::calc_ci.pielou_evenness(data.frame()), "Wrong data class")
  expect_error(b3gbi:::calc_ci.ab_rarity(data.frame(), indicator_df), "Wrong data class")
  expect_error(b3gbi:::calc_ci.area_rarity(data.frame(), indicator_df), "Wrong data class")
  expect_error(b3gbi:::calc_ci.spec_occ(data.frame(), indicator_df), "Wrong data class")
  expect_error(b3gbi:::calc_ci.spec_range(data.frame(), indicator_df), "Wrong data class")
  expect_error(b3gbi:::calc_ci.hill0(data.frame(), indicator_df), "Wrong data class")
})
...
test_that("add_ci edge cases and warnings", {
  ...
  testthat::with_mocked_bindings(
    ...
    {
      expect_warning(
        b3gbi::add_ci(mock_indicator, bootstrap_level = "cube"),
  ...
  expect_warning(
    b3gbi::add_ci(ci_indicator, overwrite = FALSE),
...
test_that("calc_ci density methods area validation", {
  ...
  expect_error(b3gbi:::calc_ci.occ_density(x_occ, indicator_df), "missing or invalid in the data attributes")
  ...
  expect_error(b3gbi:::calc_ci.spec_richness_density(x_spec, indicator_df), "missing or invalid in the data attributes")
})

test_that("calc_ci.default warning", {
  expect_warning(b3gbi::calc_ci("invalid", data.frame()), "calc_ci does not know how to handle object of class")
})
...
test_that("calc_ci.spec_occ and spec_range empty results", {
  ...
  testthat::with_mocked_bindings(
    ...
    {
      # Test spec_occ
      res_occ <- b3gbi:::calc_ci.spec_occ(mock_data, indicator_df, num_bootstrap = 10)
      ...
      res_range <- b3gbi:::calc_ci.spec_range(mock_range, indicator_df, num_bootstrap = 10)
...
test_that("calc_ci.occ_density and spec_richness_density valid area", {
  ...
  testthat::with_mocked_bindings(
    ...
    .package = "b3gbi",
    {
       res <- b3gbi:::calc_ci.occ_density(x_occ, indicator_df, num_bootstrap = 10)
  ...
  testthat::with_mocked_bindings(
    ...
    .package = "b3gbi",
    {
      res <- b3gbi:::calc_ci.spec_richness_density(x_spec, indicator_df, num_bootstrap = 10)
      expect_true("ll" %in% names(res))
    }
  )
})
...
test_that("calc_ci evenness methods trigger core", {
  ...
  testthat::with_mocked_bindings(
    ...
    .package = "b3gbi",
    {
      res_w <- b3gbi:::calc_ci.williams_evenness(x)
      expect_true("ll" %in% names(res_w))
      
      class(x) <- c("pielou_evenness", "data.frame")
      res_p <- b3gbi:::calc_ci.pielou_evenness(x)
...
test_that("calc_ci.ab_rarity and area_rarity coverage", {
  ...
  testthat::with_mocked_bindings(
    ...
    .package = "b3gbi",
    {
      ...
      res_ab <- b3gbi:::calc_ci.ab_rarity(x_ab, indicator_df)
      expect_true("ll" %in% names(res_ab))
      
      # area_rarity
      x_area <- x
      class(x_area) <- c("area_rarity", class(x))
      res_area <- b3gbi:::calc_ci.area_rarity(x_area, indicator_df)
