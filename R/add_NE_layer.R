add_NE_layer <- function(layer_name, scale, latlong_extent) {
  ...
  all_layers_info <- rbind(
    rnaturalearth::df_layers_physical %>% dplyr::mutate(category = "physical", type = layer),
    rnaturalearth::df_layers_cultural %>% dplyr::mutate(category = "cultural", type = layer)
  )
  ...
  if (nrow(layer_info) == 0) {
    stop(paste0("Layer '", layer_name, "' not found or not available at scale '", scale, "'. Please check rnaturalearth::df_layers_physical and rnaturalearth::df_layers_cultural."))
  }
  ...
  layer_raw <- tryCatch({
    ...
    if (grepl("the file .* seems not to exist", e, ignore.case = TRUE) ||
      ...
      message(paste0("Attempting to download '", layer_name, "' data due to previous load error."))
