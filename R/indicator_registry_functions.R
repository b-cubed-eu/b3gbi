...
backup_registered_indicators <- function(suppress_prompt = "FALSE") {
  ...
  # Path to backup file
  backup_fname <- system.file("data",
    "available_indicators.rda.old",
    package = "b3gbi"
  )
  ...
  if (suppress_prompt == "TRUE" || !file.exists(backup_fname)) {
    ...
    check_if_certain <- readline(
      prompt = paste("Warning: This will overwrite the existing file ",
        backup_fname,
  ...
  if (check_if_certain != "yes") {
    ...
    message(
      paste(
        "Registered indicators backed up to ",
        backup_fname,
        sep = ""
      )
    )
...
register_indicator <- function(indicator_class,
  ...
  if (is.null(map_wrapper) && is.null(ts_wrapper)) {
    stop(
      paste0(
        ...
        "_map or ",
        indicator_class,
        "_ts in package namespace.\n",
        "Before registering an indicator, you must include at least",
        "one exported and documented wrapper function for user access.\n",
        "Check that they are correctly named and that you have run",
        "devtools::document()."
      )
    )
  }
  ...
  if (is.null(map_function_arguments)) {
    ...
    stopifnot_error(
      "map_function_arguments must be a list",
  ...
  if (is.null(ts_function_arguments)) {
    ...
    stopifnot_error(
      "ts_function_arguments must be a list",
  ...
  if (!inherits(e$available_indicators, "available_indicators")) {
    class(e$available_indicators) <- append(
  ...
  if (check_if_apply_now == "yes") {
    ...
    message(
      paste(
        "Changes saved but not applied. Use load_registered_indicators()",
        "to apply."
      )
    )
  }
...
deregister_indicator <- function(indicator_number,
  ...
  check_if_certain <- readline(
    prompt = paste(
      "Are you sure you want to deregister class ",
      i_class,
      "? (yes/no) "
    )
  )
