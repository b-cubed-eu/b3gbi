...
#'
#' @param level  The desired geographic scale: "country", "continent", or
#'  "world".
...
get_NE_data <- function(level, region, ne_type, ne_scale, output_crs) {
  ...
  if (ne_scale == "large" && ne_type == "tiny_countries") {
    stop(
      paste(
        "tiny_countries are only available for medium (50 km) or small",
        "(110 km) scale maps"
      )
    )
  }
...
#'  various biodiversity indicators on a spatial grid or as a time series.
#'  It prepares the data, creates a grid, calculates indicators, and formats the
...
#'    rarefaction).
#' @param dim_type Dimension to calculate indicator over (time: 'ts', or space:
#'  'map')
...
#' @param cell_size Length of grid cell sides, in km. (Default: 10 for country,
#'  100 for continent or world)
#' @param level Spatial level: 'cube', 'continent', 'country', 'world',
...
#' @param output_crs The CRS you want for your calculated indicator. (Leave
#'  blank to let the function choose a default based on grid reference system)
#' @param first_year Exclude data before this year. (Uses all data in the cube
#'  by default.)
#' @param last_year Exclude data after this year. (Uses all data in the cube
#'  by default.)
#' @param spherical_geometry If set to FALSE, will temporarily disable spherical
#'  geometry while the function runs. Should only be used to solve specific
#'  issues. (Default is TRUE)
#' @param make_valid Calls st_make_valid() from the sf package. Increases
#'  processing time but may help if you are getting polygon errors.
#'  (Default is FALSE).
...
compute_indicator_workflow <- function(data,
  ...
  if (!inherits(data, "sim_cube")) {
    ...
    cell_size_units <- stringr::str_extract(
      data$resolutions, "(?<=[0-9,.]{1,6})[a-z]*$"
    )
