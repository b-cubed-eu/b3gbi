...
#'
#' This function takes a vector of MGRS (Military Grid Reference System) cell
#' codes and determines the most likely UTM (Universal Transverse Mercator)
#' coordinate reference system (CRS) in EPSG format. It extracts the UTM zone
#' and determines the hemisphere (North or South) based on the letter codes.
#'
#' @param cellcodes A character vector of MGRS grid cell codes (e.g., "32UMA",
#'  "33TWE").
#'
#' @return A character string representing the most probable EPSG code for the
#'  given cell codes, for example, `"EPSG:32632"`.
#'
#' @examples
...
get_crs_for_mgrs <- function(cellcodes) {
  ...
  utm_zone <- as.numeric(stringr::str_extract(cellcodes,
                                              "(^[0-9]*(?=[A-Z]{3}))"))
