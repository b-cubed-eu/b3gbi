...
stopifnot_error <- function(err_message, ...) {
  ...
  for (i in seq_len(n)) {
    ...
    if (!(is.logical(condition) && all(!is.na(condition)) && all(condition))) {
      stop(structure(
        list(message = err_message),
...
#' Function to return custom error messages based on the input code and object
#' classes. It uses stopifnot_error to return the custom message.
#'
#' @noRd
err_msgs <- function(obj, err_code, obj_class1, obj_class2 = NULL) {
  ...
  # Validate and select the error code, ensuring it matches one of the provided
  # options
  ...
  if (err_code == "cls_int") {
    ...
    if (is.null(obj_class2)) {
      ...
      condition <- inherits(obj, c(obj_class1))
    } else {
      # If obj_class2 is provided, perform both checks
      condition <- inherits(obj, c(obj_class1)) &
        rlang::inherits_any(obj, c(obj_class2))
    }

    # Issue an error message if the condition for "cls_str_int" is not met
    stopifnot_error(paste(
      "There is a problem with the class and/or structure of your data.\n",
...
specpool <- function(x, pool, smallsample = TRUE) {
  ...
  groups <- table(pool)
  inds <- names(groups)
  S <- var.chao <- chao <- var.jack1 <- jack.1 <- jack.2 <-
    var.boot <- bootS <- rep(NA, length(inds))
